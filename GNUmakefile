...
iter11:
	${WORKDIR}/cmd/tests/shortenertest_v2 -test.v -test.run=^TestIteration11$$ -binary-path=${BINARY_NAME} -source-path=${WORKDIR}/internal -server-port=8080 -file-storage-path=./storage.json -database-dsn='postgres://murl:murl_pswd@localhost:5530/murl_00' 2>&1
...
iter16:
	${WORKDIR}/cmd/tests/shortenertest_v2 -test.v -test.run=^TestIteration16$$ -binary-path=${BINARY_NAME} -source-path=${WORKDIR}/internal -server-port=8080 -file-storage-path=./storage.json -database-dsn='postgres://murl:murl_pswd@localhost:5530/murl_00'
