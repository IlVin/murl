...
//go:generate $GOPATH/bin/mockgen -source=repository/pgc/pg_instance.go                -destination=mocks/pgc_pginstance_mock.go       -package=mocks
//go:generate $GOPATH/bin/mockgen -source=repository/pgc/instance/pg_instance_impl.go  -destination=mocks/pgc_pginstance_impl_mock.go  -package=mocks
...
//go:generate $GOPATH/bin/mockgen -source=model/model.go                               -destination=mocks/model_mock.go                -package=mocks
//go:generate $GOPATH/bin/mockgen -source=model/event/event.go                         -destination=mocks/event_mock.go                -package=mocks
//go:generate $GOPATH/bin/mockgen -source=model/jwtmanager/jwtmanager.go               -destination=mocks/jwtmanager_mock.go           -package=mocks
...
//go:generate $GOPATH/bin/mockgen -source=handlers/middleware/session.go               -destination=mocks/session_mock.go              -package=mocks
