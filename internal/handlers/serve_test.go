...
func TestAddReqMux(t *testing.T) {
	...
	router := NewMuxRouter(cfg, hndlrs)
...
func TestGetReqMux(t *testing.T) {
	cfg := config.GetConfig()
	drv := repository.NewInMemoryDrv(cfg)
	store := repository.NewStore(cfg, drv)
	service := service.NewService(cfg, store)
...
func TestAddReqChi(t *testing.T) {
	...
	service := service.NewService(cfg, store)
	hndlrs := NewHandlers(cfg, service)
	router := NewChiRouter(cfg, hndlrs)
	srv := httptest.NewServer(router)
	defer srv.Close()
...
func TestGetReqChi(t *testing.T) {
	cfg := config.GetConfig()
	drv := repository.NewInMemoryDrv(cfg)
	store := repository.NewStore(cfg, drv)
	service := service.NewService(cfg, store)
